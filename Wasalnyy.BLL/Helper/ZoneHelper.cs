...
namespace Wasalnyy.BLL.Helper
{
    ...
    {
        public static (decimal MinLat, decimal MaxLat, decimal MinLng, decimal MaxLng) GetBoundingBoxAsync(List<Coordinate> coordinates)
        {
            return (coordinates.Min(c => c.Lat), coordinates.Max(c => c.Lat), coordinates.Min(c => c.Lng), coordinates.Max(c => c.Lng));
        }

        public static bool IsPointInPolygon(decimal lat, decimal lng, List<Coordinate> polygon)
        {
            bool inside = false;
